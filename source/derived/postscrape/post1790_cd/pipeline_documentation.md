...
- [geo_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_standardized_CD_post1790.csv): continental debt files with final geographical classification

**Outputs (to check validity of cleaning process)**: 

- [change_df_CD](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/check/town_occ_agg_check.csv): crosswalk mapping aggregation of multiple towns/occupations (raw data) to one town/occupation ([geo_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_standardized_CD_post1790.csv))
...
6. Our final results are in [geo_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_standardized_CD_post1790.csv)

`town`, `occupation` and `state`  are given columns from the raw data but post step 2

`new_town`, `new_county`, `new_state`, `country`, `geo_level ` are columns created post-cleaning and represent the location of an individual

|      | town                      | state | occupation | new_town      | county          | new_state | country | geo_level |
| ---: | :------------------------ | :---- | :--------- | :------------ | :-------------- | :-------- | :------ | :-------- |
...
print(CD_all[['town', 'state', 'occupation', 'new_town', 'county', 'new_state', 'country', 'geo_level']].loc[[0,2,390, 2001]].to_markdown())
```
...
- [geo_name_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_name_standardized_CD_post1790.csv): [geo_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_standardized_CD_post1790.csv) with cleaned names
...
1. First, we import [geo_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_standardized_CD_post1790.csv) 
2. The names in [geo_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_standardized_CD_post1790.csv) can be quite messy for various reasons
...
7. we also create [geo_name_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_name_standardized_CD_post1790.csv), which contains debt data, the original name and the new (cleaned) name
...
|      | Fn_Fix   | Ln_Fix   | new_town   | county          | new_state | country | geo_level | Match Index | Match Status              |
| ---: | :------- | :------- | :--------- | :-------------- | :-------- | :------ | :-------- | :---------- | :------------------------ |
...
print(pd.read_csv('scrape_tools/name_list_scraped.csv', index_col = 0).loc[[1, 9, 21]][['Fn_Fix', 'Ln_Fix', 'new_town', 'county', 'new_state', 'country','geo_level', 'Match Index', 'Match Status']].to_markdown())
...
1. First, we import [geo_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_standardized_CD_post1790.csv), [name_list_scraped.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/cleaning_CD/scrape_tools/name_list_scraped.csv), [scrape_results.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/cleaning_CD/clean_tools/scrape_results.csv) and [name_list.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/cleaning_CD/clean_tools/name_list.csv)

2. In **Adding Missing Occupations**, there are cases where an occupation is part of a name in [geo_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_standardized_CD_post1790.csv), but the occupation is not listed in the occupation column. we amend this by adding the occupation
...
3. There are two steps in **Merge Data** to create the aggregate dataset we will work with
   ...
   1. In [geo_standardized_CD_post1790.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/data_clean/geo_standardized_CD_post1790.csv) (variable `CD_clean`), one debtholder entry name (`Name` column) sometimes corresponds to multiple names. we merge it with [name_list.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/cleaning_CD/clean_tools/name_list.csv) (variable `name_df`), which maps one debtholder entry name (`Name`) to however many actual names are there are (values listed in `Name_Fix`). Note that now, the same debtholder entry may be linked to multiple "names". Here is an example:
      ...
      3. |      | Name                                | Fn_Fix  | Ln_Fix    | county           | new_state | country | geo_level |
         | ---: | :---------------------------------- | :------ | :-------- | :--------------- | :-------- | :------ | :-------- |
         ...
         |      | Name                                | state_data | state_data_index | Name_Fix                            | Fn_Fix  | Ln_Fix    | county           | new_state | country | geo_level |
         | ---: | :---------------------------------- | :--------- | ---------------: | :---------------------------------- | :------ | :-------- | :--------------- | :-------- | :------ | :-------- |
   ...
   2. Next, we merge our merged table with `name_list_scraped.csv` (variable `scraped_names`) so that our table includes information on the census match results from Ancestry.com. Here is an example
      ...
      2. `scraped_names`
         ...
         |      | Fn_Fix  | Ln_Fix | county           | new_state | country | geo_level | Match Index | Match Status   |
         | ---: | :------ | :----- | :--------------- | :-------- | :------ | :-------- | ----------: | :------------- |
         |    8 | Francis | Brown  | New Haven County | CT        | US      | town      |           7 | Complete Match |

      3. merged table (variable `CD_merged_mind`)

      4. |      | Name          | state_data | state_data_index | Name_Fix      | Fn_Fix  | Ln_Fix | Full Search Name | county           | new_state | country | geo_level | Match Index | Match Status   |
         | ---: | :------------ | :--------- | ---------------: | :------------ | :------ | :----- | :--------------- | :--------------- | :-------- | :------ | :-------- | ----------: | :------------- |
...
4. Next, we begin the process of grouping names that represent the same identity together in the **Group Names - Using Ancestry.com Matches** section. First, we take advantage of the fact that sometimes, Ancestry.com matches two individuals with slightly different names to the same identity (variable `Match Index`)
   ...
   2. There are some manual corrections we have to make, when we don't want to the longest name to be the representative name
      ...
      2. This is important because sometimes, multiple names have different values for the aforementioned characteristics (for example, one name might have location at `geo_level` county, while another has location at `geo_level` town  - see tables below)
      3. We pick values for whatever characteristic is most detailed (in the above example, we pick `geo_level` town)
   ...
   3. Here's an example for a case where we don't have to change the location (`Richard Woottan`) and a case where we do (`Gassaway Watkins`)
      ...
      1. |      | Name                                                   | state_data | state_data_index | Name_Fix                                               | Fn_Fix   | Ln_Fix  | county              | new_state | country | geo_level | Match Index | Match Status   | Group Name       | Group Town | Group County        | Group State | Group Country | Group Name Type | Group Match Index | Group Match Status |
         | ---: | :----------------------------------------------------- | :--------- | ---------------: | :----------------------------------------------------- | :------- | :------ | :------------------ | :-------- | :------ | :-------- | ----------: | :------------- | :--------------- | :--------- | :------------------ | :---------- | :------------ | :-------------- | ----------------: | :----------------- |
...
5. Next, we have a list of names (manually curated)  that represent the same identity but are slightly misspelled that we deal with in the **Group Names - Fuzzy Matching**. These are names that the Ancestry.com APwe could not find matches for, or somehow missed. we import [name_agg.csv](https://github.com/liaochris/SPEOC-pt-1/blob/main/cleaning_CD/clean_tools/name_agg.csv) (variable `rep_names`)
   ...
   3. Here's an example of two entries in `rep_names` and the subsequent result. Notice how John Salter represents the identities of multiple individuals, so we specify a location
      ...
      3. |      | Name                           | state_data | state_data_index | Name_Fix                       | Fn_Fix | Ln_Fix | county              | new_state | country | geo_level | Match Index | Match Status              | Group Name    | Group Town       | Group County        | Group State | Group Country | Group Name Type | Group Match Index | Group Match Status        |
         | ---: | :----------------------------- | :--------- | ---------------: | :----------------------------- | :----- | :----- | :------------------ | :-------- | :------ | :-------- | :---------- | :------------------------ | :------------ | :--------------- | :------------------ | :---------- | :------------ | :-------------- | :---------------- | :------------------------ |
...
6. Another group of people we want to group together are those that have the same name, but different places of residence, which we handle in **Group Names - same name, within state**.These are individuals who have the same name and reside in the same state, but have different locations. 
   ...
   1. In particular, these names, collectively, must have
      ...
      2. At least one `geo_level` (county + town, county + state, or town + state)
   ...
   4. Here's an example. Note that `Abner Johnson` has overlapping, as opposed to identical Ancestry.com match data because in the case where we have town information, we can identify his census record more precisely
      ...
      2. |      | Group Name      | Group State | Group County | Group Town | Group Name Type |
         ...
         |      | Name            | state_data | state_data_index | Name_Fix        | Fn_Fix  | Ln_Fix  | county           | new_state | country | geo_level | Match Index | Match Status              | Group Name      | Group Town   | Group County     | Group State | Group Country | Group Name Type | Group Match Index | Group Match Status |
         | ---: | :-------------- | :--------- | ---------------: | :-------------- | :------ | :------ | :--------------- | :-------- | :------ | :-------- | :---------- | :------------------------ | :-------------- | :----------- | :--------------- | :---------- | :------------ | :-------------- | ----------------: | :----------------- |
...
11. Another group of people we want to group together are those that have the same name, but different places of residence, which we handle in **Group Names - incorrect states** This occurs because when we impute values for the `state` column, sometimes the incorrect value is imputed (example: CT debt file, individual from MA but missing `state` column value but we impute CT as the state). In our example, we know this is an incorrect value because another individual from `MA` has the same name, an ancestry.com match, and (often times) also holds CT debt in the same file. 
    ...
    1. In this case, we set the group characteristics to be whatever data is most specific (most specific `geo_level` for location)
    ...
    4. When both names only have location `geo_level` at the state value, we input manually which state to aggregate data at (based on manual inspection)

       1. Otherwise, we pick the most specific `geo_level`, as per our usual procedure. See example below
