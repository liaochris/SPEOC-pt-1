...
OUTDIR = Path("output/scrape/ancestry_town_population_scraper")
...
def Main():
    ...
    towns = LoadCDTowns()
    town_pops = ScrapeLocations(browse_format=False, towns_l=towns)
    SaveResults(town_pops, OUTDIR / "town_pops.csv")
...
def SaveResults(town_pops, path):
...
def FormatBrowseResults(town_pops):
    ...
    print(towns_df)
    towns_df.to_csv(OUTDIR / "town_pops_browse_clean.csv", index=False)
...
def FixScrapeErrors():
    ...
    csv_town = pd.read_csv(OUTDIR / "town_pops_browse_clean.csv")
    ...
    csv_town.to_csv(OUTDIR / "town_pops_browse_clean.csv", index=False)
